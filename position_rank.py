...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 12 10:15:03 2021
...
def position_rank(sentence, tokenizer, alpha=0.85, window_size=6, num_keyphrase=10, lang="en"):
    ...
    Konum sıralaması, anahtar kelimeyi cümleden çıkarmak için bir yöntemdir.

    This method is allowed any language if you provide tokenizer
    that tokenize language you wanna apply position rank.
    Belirteçleyici sağlarsanız, bu yönteme herhangi bir dilde izin verilir.
    pozisyon sıralamasını uygulamak istediğiniz dili belirtmelisiniz.

    In the oroginal paper, authors used 'title' and 'abstract' of scholarly documents.
    Orijinal makalede yazarlar, bilimsel belgelerin 'başlık' ve 'soyutunu' kullanmışlardır.
    Original paper is here: http://aclweb.org/anthology/P/P17/P17-1102.pdf

    Args:
      sentence: Text concatenated title and abstract.
      tokenizer: Object that tokenize sentence. -> Cümleyi simgeleyen nesne.
      ...
      alpha: Damping parameter. This allows 'teleport' operation into another node in the graph. -> Sönüm parametresi. Bu, grafikteki başka bir düğüme 'ışınlanma' işlemine izin verir.
      window_size: Size of woindow for co-occurece word. ->Birlikte meydana gelen kelime için pencerenin boyutu
      num_keyphrase: Number of keyphrase which method will return. ->Yöntemin döndüreceği anahtar sözcük sayısı.
      lang: Target language. ->Hedef Dil
    Returns:
      Keyphrase list. Length of list is decided by 'num_keyphrase' param. ->Anahtar sözcük listesi. Listenin uzunluğuna 'num_keyphrase' parametresi ile karar verilir.
    """
    if lang == "en":
        stem = porter.stem
    else:
    ...
    adjancency_matrix = np.zeros((n, n)) # N ' e N lik sıfırlarla oluşturlmuş bir kelime uzay vektörü oluşturuyoruz.
    word2idx = {w: i for i, w in enumerate(unique_word_list)}
    """
    İngilizcede enumerate kelimesi 'numaralandırmak' anlamına gelir. 
    enumerate() fonksiyonunun görevi de kelimenin bu anlamıyla aynıdır. Yani bu fonksiyonu kullanarak nesneleri numaralandırabiliriz.
    """
    p_vec = np.zeros(n)
    # store co-occurence words
    co_occ_dict = {w: [] for w in unique_word_list}

    # 1. initialize  probability vector ->Olasılık vektörü başlar
    ...
    # 2. create adjancency matrix from co-occurence word ->birlikte bulunma kelimesinden bitişiklik matrisi oluşturulur
    ...
    lambda_val = 1.0
    loop = 0
    # compute final principal eigenvector ->nihai ana özvektörü hesapla
    ...
    # score original words and phrases ->orijinal kelimeleri ve cümleleri puanlanır
